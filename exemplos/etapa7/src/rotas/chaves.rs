...
pub fn rotas() -> Vec<Route> {
    routes![listar_chaves, pagina_criar_chave, criar_chave, editar_chave_template, editar_chave]
}
...
#[get("/editar?<name>&<color>")]
async fn editar_chave_template(database: &State<Database>, name: &str, color: &str) -> Template {
    let lista_chaves = ColecaoChaves::default(database).listar_chaves().await;
